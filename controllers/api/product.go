...
func (con ProductController) Category(c *gin.Context) {
	//分类id
	cateId, _ := models.Str2Int(c.Param("id"))
	//当前页
	...
	db.MySQLDB.Where("id=?", cateId).Find(&currentCate)
	subCate := []models.GoodsCate{}
	var tempSlice []int
	if currentCate.Pid == 0 {
		//获取二级分类
		db.MySQLDB.Where("pid=?", currentCate.Id).Find(&subCate)
		for i := 0; i < len(subCate); i++ {
			tempSlice = append(tempSlice, subCate[i].Id)
		}
	} else {
		//兄弟分类
		db.MySQLDB.Where("pid=?", currentCate.Pid).Find(&subCate)
	}
	tempSlice = append(tempSlice, cateId)
	where := "cate_id in ?"
	goodsList := []models.Goods{}
	db.MySQLDB.Where(where, tempSlice).Offset((page - 1) * pageSize).Limit(pageSize).Find(&goodsList)

	//获取总数量
	var count int64
	db.MySQLDB.Where(where, tempSlice).Table("goods").Count(&count)

	tpl := "api/product/list.html"
...
func (con ProductController) Detail(c *gin.Context) {
	...
	db.MySQLDB.Where("id in ?", relationIds).Select("id,title,price,goods_version").Find(&relationGoods)
	...
	db.MySQLDB.Where("id in ?", giftIds).Select("id,title,price,goods_version").Find(&goodsGift)

	//4、获取关联颜色 GoodsColor
	goodsColor := []models.GoodsColor{}
	goods.GoodsColor = strings.ReplaceAll(goods.GoodsColor, "，", ",")
	colorIds := strings.Split(goods.GoodsColor, ",")
	db.MySQLDB.Where("id in ?", colorIds).Find(&goodsColor)
	...
	db.MySQLDB.Where("id in ?", fittingIds).Select("id,title,price,goods_version").Find(&goodsFitting)

	//6、获取商品关联的图片 GoodsImage
	goodsImage := []models.GoodsImage{}
	db.MySQLDB.Where("goods_id=?", goods.Id).Limit(6).Find(&goodsImage)
...
func (con ProductController) GetImgList(c *gin.Context) {
	...
	goodsId, err1 := models.Str2Int(c.Query("goods_id"))
	colorId, err2 := models.Str2Int(c.Query("color_id"))

	//查询商品图库信息

	goodsImageList := []models.GoodsImage{}
	err3 := db.MySQLDB.Where("goods_id=? AND color_id=?", goodsId, colorId).Find(&goodsImageList).Error
	...
	if err1 != nil || err2 != nil || err3 != nil {
		...
		if len(goodsImageList) == 0 {
			db.MySQLDB.Where("goods_id=?", goodsId).Find(&goodsImageList)
		}
		c.JSON(200, gin.H{
			"success": true,
