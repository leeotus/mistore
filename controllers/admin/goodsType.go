...
func (con GoodsTypeController) DoAdd(c *gin.Context) {
	...
	status, err1 := models.Str2Int(c.PostForm("status"))
	...
	err := db.MySQLDB.Create(&goodsType).Error
	if err != nil {
...
func (con GoodsTypeController) DoEdit(c *gin.Context) {
	...
	title := strings.Trim(c.PostForm("title"), " ")
	description := strings.Trim(c.PostForm("description"), " ")
	status, err2 := models.Str2Int(c.PostForm("status"))
	...
	if title == "" {
		con.Error(c, "商品类型的标题不能为空", "/admin/goodsType/edit?id="+models.Int2Str(id))
	}
	goodsType := models.GoodsType{Id: id}
	db.MySQLDB.Find(&goodsType)
	...
	err3 := db.MySQLDB.Save(&goodsType).Error
	if err3 != nil {
		con.Error(c, "修改数据失败", "/admin/goodsType/edit?id="+models.Int2Str(id))
	} else {
